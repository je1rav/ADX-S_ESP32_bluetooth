/*
 * si5351.c - Si5351 library for esp-idf (ESP32)
 *  
 * si5351_init(), si5351_write_bulk(), si5351_write(), si5351_read() 
 * are modified to use with esp-idf (ESP32)  by Hitoshi Kawaji (JE1RAV) <je1rav@gmail.com> on 2023.
...
uint32_t pll_calc(uint32_t freq, struct Si5351RegSet *reg, int32_t correction)
{
	...
	//ref_freq += (uint32_t)((double)(correction / 10000000.0) * (double)ref_freq);             //  by JE1RAV
	ref_freq += (int32_t)((double)(correction / 10000000.0) * (double)ref_freq);             //  by JE1RAV
...
uint8_t si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	for(int i = 0; i < (bytes - 1); i++)
	{
 	...
 	ret = i2c_master_write_to_device(I2C_MASTER_NUM, SI5351_BUS_ADDR, buf, sizeof(buf), I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
	return 0;
}
...
{
    size_t len=1;
	return i2c_master_write_read_device(I2C_MASTER_NUM, SI5351_BUS_ADDR, &addr, 1, data, len, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
	return 0;	
}
